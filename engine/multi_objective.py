...
def build_multi_objective_scores(
    ...
    objective = {
        ...
        "coherence": _clamp(base_coherence + foresight * 0.05 + market_resonance * 0.02 + repair_confidence * 0.05 + portfolio_fit * 0.03 - repair_penalty),
        ...
        "pacing": _clamp(base_pacing + pressure_clock * 0.05 + exploration_bias * 0.03 + bingeability * 0.03 + repair_confidence * 0.03 + diversity_pressure * 0.05),
        ...
        "information_design": _clamp(float(information.get("dramatic_irony", 5)) / 10.0 * 0.22 + float(retention_state.get("information_gap", 5)) / 10.0 * 0.14 + causal_report.get("checks", {}).get("cliffhanger_alignment", 0.0) * 0.15 + exploration_bias * 0.08 + market_resonance * 0.16 + repair_confidence * 0.17 + portfolio_fit * 0.08 - repair_penalty * 0.5),
        ...
        "stability": _clamp(base_stability + foresight * 0.05 + market_resonance * 0.05 + exploration_bias * 0.04 + release_confidence * 0.04 + repair_confidence * 0.06 + portfolio_fit * 0.05 - overused_penalty - repair_penalty - shared_risk_alert * 0.05),
    }
    return objective
