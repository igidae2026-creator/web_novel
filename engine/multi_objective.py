...
def build_multi_objective_scores(
    ...
    objective = {
        ...
        "long_run_sustainability": _clamp(float(serialization.get("sustainability", 5)) / 10.0 * 0.50 + float(rewards.get("expectation_alignment", 5)) / 10.0 * 0.15 + reader_trust * 0.10 + release_confidence * 0.07 + portfolio_fit * 0.07 + portfolio_coordination * 0.06 + strategy_coverage * 0.06 + cadence_guard * 0.02 + release_guard * 0.02 + (1.0 - shared_risk_alert) * 0.02 + (1.0 - novelty_debt) * 0.02 - max(0.0, cross_track_risk - 0.7) * 0.01),
        ...
        "stability": _clamp(base_stability + foresight * 0.05 + market_resonance * 0.05 + exploration_bias * 0.03 + release_confidence * 0.04 + repair_confidence * 0.03 + closure_score * 0.05 + strategy_coverage * 0.04 + portfolio_fit * 0.04 + learning_confidence * 0.05 + portfolio_coordination * 0.06 - overused_penalty - repair_penalty - shared_risk_alert * 0.03 - max(0.0, cross_track_risk - 0.6) * 0.02 - max(0.0, pattern_crowding - 0.6) * 0.01 - max(0.0, market_overlap - 0.6) * 0.01),
    }
    return objective
