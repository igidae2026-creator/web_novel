...
def build_multi_objective_scores(
    ...
    objective = {
        ...
        "coherence": _clamp(base_coherence + foresight * 0.05 + market_resonance * 0.02 + repair_confidence * 0.03 + closure_score * 0.05 + strategy_coverage * 0.05 + portfolio_fit * 0.03 + learning_confidence * 0.03 + portfolio_coordination * 0.03 - repair_penalty),
        "character_persuasiveness": _clamp(scores.get("character_score", 0.5) * 0.60 + scores.get("emotion_density", 0.5) * 0.18 + causal_report.get("checks", {}).get("goal_pressure", 0.0) * 0.12 + repair_confidence * 0.04 + closure_score * 0.06 + strategy_coverage * 0.04),
        "pacing": _clamp(base_pacing + pressure_clock * 0.05 + exploration_bias * 0.03 + bingeability * 0.03 + repair_confidence * 0.02 + closure_score * 0.04 + diversity_pressure * 0.04 + cadence_guard * 0.03 + release_schedule_health * 0.05 + portfolio_coordination * 0.03 - max(0.0, cadence_pressure - 0.6) * 0.01),
        "retention": _clamp(base_retention + pressure_clock * 0.05 + market_resonance * 0.05 + exploration_bias * 0.02 + bingeability * 0.03 + reader_trust * 0.03 + portfolio_fit * 0.05 + learning_confidence * 0.03 + release_guard * 0.03 + release_schedule_health * 0.075 + portfolio_coordination * 0.04 - max(0.0, release_interference - 0.6) * 0.01),
        "emotional_immersion": _clamp(scores.get("emotion_density", 0.5) * 0.7 + causal_report.get("checks", {}).get("emotional_trace", 0.0) * 0.3 + strategy_coverage * 0.06),
        "information_design": _clamp(float(information.get("dramatic_irony", 5)) / 10.0 * 0.18 + float(retention_state.get("information_gap", 5)) / 10.0 * 0.13 + causal_report.get("checks", {}).get("cliffhanger_alignment", 0.0) * 0.14 + exploration_bias * 0.08 + market_resonance * 0.15 + repair_confidence * 0.09 + closure_score * 0.09 + strategy_coverage * 0.11 + portfolio_fit * 0.05 + learning_confidence * 0.08 + novelty_guard * 0.04 + portfolio_coordination * 0.04 - repair_penalty * 0.5),
        "emotional_payoff": _clamp(scores.get("emotion_density", 0.5) * 0.75 + scores.get("payoff_score", 0.5) * 0.25 + portfolio_coordination * 0.02),
        "long_run_sustainability": _clamp(float(serialization.get("sustainability", 5)) / 10.0 * 0.49 + float(rewards.get("expectation_alignment", 5)) / 10.0 * 0.15 + reader_trust * 0.10 + release_confidence * 0.07 + portfolio_fit * 0.07 + portfolio_coordination * 0.06 + release_schedule_health * 0.075 + strategy_coverage * 0.06 + cadence_guard * 0.02 + release_guard * 0.02 + (1.0 - shared_risk_alert) * 0.02 + (1.0 - novelty_debt) * 0.02 - max(0.0, cross_track_risk - 0.7) * 0.01),
        ...
        "stability": _clamp(base_stability + foresight * 0.05 + market_resonance * 0.05 + exploration_bias * 0.03 + release_confidence * 0.04 + repair_confidence * 0.03 + closure_score * 0.05 + strategy_coverage * 0.04 + portfolio_fit * 0.04 + learning_confidence * 0.05 + release_schedule_health * 0.06 + portfolio_coordination * 0.06 - overused_penalty - repair_penalty - shared_risk_alert * 0.03 - max(0.0, cross_track_risk - 0.6) * 0.02 - max(0.0, pattern_crowding - 0.6) * 0.01 - max(0.0, market_overlap - 0.6) * 0.01),
    }
    return objective
